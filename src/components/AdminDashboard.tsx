...
const STATUS_COLORS: Record<Order['status'], { bg: string; color: string; label_tr: string; label_en: string; label_ru: string }> = {
  ...
  delivered: { bg: '#E8F0E8', color: '#2C5F2E', label_tr: 'Teslim Edildi', label_en: 'Delivered', label_ru: 'Доставлено' },
...
function AdminLogin({ onLogin, t }: { onLogin: () => void; t: ReturnType<typeof useT> }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#FAF7F2' }}>
      <div className="w-full max-w-sm p-8 rounded-3xl" style={{ background: '#FFFDF9', border: '1.5px solid #E5DDD0', boxShadow: '0 8px 40px rgba(0,0,0,0.08)' }}>
        <div className="text-center mb-8">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: green }}>
            <span style={{ color: gold, fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: '1.3rem' }}>F</span>
          </div>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.5rem', color: '#1A1A1A' }}>{t('admin_login')}</h1>
        </div>
        <div className="space-y-4">
          <div className="relative">
            <input
              ...
              style={{ background: '#FAF7F2', border: `1.5px solid ${error ? '#C0392B' : '#E5DDD0'}`, fontFamily: "'DM Sans', sans-serif", color: '#1A1A1A' }}
            />
            <button onClick={() => setShow(!show)} className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer" style={{ color: '#8A8A8A' }}>
              {show ? <EyeOff size={16} /> : <Eye size={16} />}
            </button>
          </div>
          {error && <p style={{ color: '#C0392B', fontSize: '12px', fontFamily: "'DM Sans'" }}>{t('admin_wrong_pass')}</p>}
          <button
            onClick={() => pass === ADMIN_PASSWORD ? onLogin() : setError(true)}
            className="w-full py-3 rounded-2xl text-sm font-semibold cursor-pointer"
            style={{ background: green, color: '#fff', fontFamily: "'DM Sans', sans-serif" }}
          >
            {t('admin_login_btn')}
          </button>
        </div>
        <p className="text-center mt-4 text-xs" style={{ color: '#8A8A8A', fontFamily: "'DM Sans'" }}>Demo: admin123</p>
      </div>
    </div>
  );
}
...
function OrdersTab({ t }: { t: ReturnType<typeof useT> }) {
  ...
  return (
    <div>
      <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.3rem', color: '#1A1A1A', marginBottom: '20px' }}>{t('admin_orders')}</h2>
      <div className="space-y-3">
        {state.orders.length === 0 && (
          <p style={{ color: '#8A8A8A', fontFamily: "'DM Sans'", textAlign: 'center', padding: '40px 0' }}>No orders yet</p>
        )}
        {state.orders.map(order => {
          const sc = STATUS_COLORS[order.status];
          return (
            <div key={order.id} className="p-5 rounded-2xl" style={{ background: '#FFFDF9', border: '1.5px solid #E5DDD0' }}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 600, color: '#1A1A1A', fontSize: '1rem' }}>{order.orderNumber}</span>
                    <span className="px-2.5 py-1 rounded-full text-xs font-medium" style={{ background: sc.bg, color: sc.color, fontFamily: "'DM Sans'" }}>
                      {getStatusLabel(order.status, state.lang)}
                    </span>
                  </div>
                  <p style={{ fontFamily: "'DM Sans'", fontSize: '14px', color: '#4A4A4A' }}>{order.customerName} · {order.customerPhone}</p>
                  <p style={{ fontFamily: "'DM Sans'", fontSize: '12px', color: '#8A8A8A', marginTop: '2px' }}>
                  ...
                  {order.subscriptionPlan && (
                    <p style={{ fontFamily: "'DM Sans'", fontSize: '12px', color: green, marginTop: '2px' }}>📦 {order.subscriptionPlan.name}</p>
                  )}
                </div>
                <div className="text-right">
                  <p style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: '1.1rem', color: '#1A1A1A' }}>₺{order.total.toLocaleString('tr-TR')}</p>
                  <p style={{ fontFamily: "'DM Sans'", fontSize: '11px', color: '#8A8A8A', marginTop: '2px' }}>{new Date(order.createdAt).toLocaleString()}</p>
                </div>
              </div>
              {/* Status change */}
...
function MealForm({ meal, onSave, onCancel }: { meal: Meal; onSave: (m: Meal) => void; onCancel: () => void }) {
  ...
  const iStyle = { width: '100%', padding: '10px 14px', borderRadius: '12px', border: '1.5px solid #E5DDD0', background: '#FAF7F2', fontFamily: "'DM Sans', sans-serif", fontSize: '13px', color: '#1A1A1A' };
  const lStyle = { fontSize: '11px', fontWeight: 600, color: '#8A8A8A', fontFamily: "'DM Sans'", textTransform: 'uppercase' as const, letterSpacing: '0.05em', display: 'block', marginBottom: '5px' };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="flex items-center gap-3 pt-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span style={{ fontFamily: "'DM Sans'", fontSize: '13px', color: '#4A4A4A' }}>Available</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={form.featured} onChange={e => f('featured', e.target.checked)} style={{ accentColor: green }} />
            <span style={{ fontFamily: "'DM Sans'", fontSize: '13px', color: '#4A4A4A' }}>Featured</span>
          </label>
        </div>
      </div>
      <div className="flex gap-3 mt-4">
        <button onClick={() => onSave(form)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold cursor-pointer" style={{ background: green, color: '#fff', fontFamily: "'DM Sans'" }}>
          <Check size={14} /> Save
        </button>
        <button onClick={onCancel} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium cursor-pointer" style={{ background: '#E5DDD0', color: '#4A4A4A', fontFamily: "'DM Sans'" }}>
          <X size={14} /> Cancel
        </button>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.3rem', color: '#1A1A1A' }}>{t('admin_menu')}</h2>
        <button onClick={() => { setAdding(true); setEditing(null); }} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold cursor-pointer" style={{ background: green, color: '#fff', fontFamily: "'DM Sans'" }}>
        ...
        {state.adminMeals.map(meal => (
          <div key={meal.id}>
            {editing === meal.id ? (
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl" style={{ background: '#FFFDF9', border: '1.5px solid #E5DDD0', opacity: meal.available ? 1 : 0.6 }}>
                ...
                <div className="flex-1 min-w-0">
                  <p style={{ fontFamily: "'DM Sans'", fontWeight: 600, color: '#1A1A1A', fontSize: '14px' }}>{meal.name}</p>
                  <p style={{ fontFamily: "'DM Sans'", fontSize: '12px', color: '#8A8A8A' }}>₺{meal.price} · {meal.calories}kcal · {meal.category}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="px-2 py-1 rounded-lg text-xs font-medium" style={{ background: meal.available ? '#E8F0E8' : '#F5F5F5', color: meal.available ? green : '#8A8A8A', fontFamily: "'DM Sans'" }}>
    ...
    <div className="p-5 rounded-2xl mb-4" style={{ background: '#F5ECD7', border: '1.5px solid #C8A97A' }}>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <button onClick={() => f('features', [...form.features, ''])} className="px-3 py-2 rounded-xl text-xs font-medium cursor-pointer" style={{ background: '#E8F0E8', color: green, fontFamily: "'DM Sans'" }}>+ Add Feature</button>
        </div>
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={!!form.popular} onChange={e => f('popular', e.target.checked)} style={{ accentColor: green }} />
            <span style={{ fontFamily: "'DM Sans'", fontSize: '13px', color: '#4A4A4A' }}>Popular (highlighted)</span>
          </label>
        </div>
      </div>
      <div className="flex gap-3 mt-4">
...
function PackagesTab({ t }: { t: ReturnType<typeof useT> }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.3rem', color: '#1A1A1A' }}>{t('admin_packages')}</h2>
        <button onClick={() => { setAdding(true); setEditing(null); }} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold cursor-pointer" style={{ background: green, color: '#fff', fontFamily: "'DM Sans'" }}>
        ...
        {state.adminPlans.map(plan => (
          <div key={plan.id}>
            {editing === plan.id ? (
              ...
              <div className="p-5 rounded-2xl" style={{ background: plan.popular ? '#2C5F2E' : '#FFFDF9', border: '1.5px solid #E5DDD0' }}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p style={{ fontFamily: "'Playfair Display', serif", fontWeight: 600, color: plan.popular ? '#FFFDF9' : '#1A1A1A', fontSize: '1rem' }}>{plan.name}</p>
                    <p style={{ fontFamily: "'DM Sans'", fontSize: '13px', color: plan.popular ? 'rgba(255,253,249,0.7)' : '#8A8A8A', marginTop: '3px' }}>
                      {plan.mealCount} meals · ₺{plan.price.toLocaleString('tr-TR')} · ₺{plan.pricePerMeal}/meal
                    </p>
                    {plan.badge && <span className="inline-block mt-2 px-2.5 py-1 rounded-full text-xs font-semibold" style={{ background: gold, color: '#fff', fontFamily: "'DM Sans'" }}>{plan.badge}</span>}
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => setEditing(plan.id)} className="w-8 h-8 rounded-xl flex items-center justify-center cursor-pointer hover:opacity-70" style={{ background: plan.popular ? 'rgba(200,169,122,0.3)' : '#F5ECD7', color: plan.popular ? gold : gold }}>
                      <Edit3 size={14} />
                    </button>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {plan.features.map((f, i) => (
                    <span key={i} className="px-2.5 py-1 rounded-lg text-xs" style={{ background: plan.popular ? 'rgba(255,253,249,0.15)' : '#E8F0E8', color: plan.popular ? 'rgba(255,253,249,0.85)' : green, fontFamily: "'DM Sans'" }}>✓ {f}</span>
                  ))}
                </div>
              </div>
            )}
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#FAF7F2' }}>
      {/* Top bar */}
      <div className="sticky top-0 z-50 border-b" style={{ background: 'rgba(250,247,242,0.97)', backdropFilter: 'blur(12px)', borderColor: '#E5DDD0' }}>
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 600, color: '#1A1A1A', fontSize: '1.1rem' }}>{t('admin_title')}</span>
          </div>
          <button onClick={handleLogout} className="px-4 py-2 rounded-xl text-sm font-medium cursor-pointer hover:opacity-70" style={{ background: '#FEE2E2', color: '#C0392B', fontFamily: "'DM Sans'" }}>
            {t('admin_logout')}
          </button>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-5 sm:px-8 py-6">
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0">
              {tabs.map(tb => (
                <button key={tb.id} onClick={() => setTab(tb.id)}
                  ...
                  style={{
                    background: tab === tb.id ? green : '#FFFDF9',
          ...
          {/* Content */}
          <div className="flex-1 min-w-0">
            <div className="p-5 sm:p-6 rounded-3xl" style={{ background: '#FFFDF9', border: '1.5px solid #E5DDD0' }}>
